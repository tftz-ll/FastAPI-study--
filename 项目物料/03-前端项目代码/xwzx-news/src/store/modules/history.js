  ...
  actions: {
    ...
    async getHistoryListApi(page = 1, pageSize = 10) {
      ...
      try {
        ...
        console.log('获取浏览历史API：开始请求', { page, pageSize });
        ...
        if (response.data.code === 200) {
          ...
          if (page === 1) {
            ...
            this.history = [...this.history, ...(list || [])];
          }
          this.page = page;
    ...
    // 加载更多浏览历史（自动判断首次加载还是翻页）
    async loadMoreHistory() {
      if (this.finished) return;
      const nextPage = this.history.length === 0 ? 1 : this.page + 1;
      await this.getHistoryListApi(nextPage, this.pageSize);
    },
